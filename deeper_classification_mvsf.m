...
for epoch=1:model.epochs
    ...
    if mod(epoch,model.update)==0
        ...
        hold on
        nn=200;
        xx = linspace(min(x(:,1))*1.1,1.1*max(x(:,1)),nn);
        yy = linspace(1.1*min(x(:,2)),1.1*max(x(:,2)),nn);
        [X,Y] = meshgrid(xx,yy);
        if addednoise==1
        ...
        %contour(X+xmean(1),Y+xmean(2),reshape(Z(:,1),100,100),'LineWidth',3)
        contour(X+xmean(1),Y+xmean(2),reshape(Z(:,1),nn,nn),[ 0.499 0.501],'LineWidth',3,'Color',repmat((model.epochs-epoch)/model.epochs,3,1))
...
%contour(X+xmean(1),Y+xmean(2),reshape(Z(:,1),100,100),'LineWidth',3)
contour(X+xmean(1),Y+xmean(2),reshape(Z(:,1),nn,nn),[ 0.499999999 0.500000001],'LineWidth',3,'Color',repmat((model.epochs-epoch)/model.epochs,3,1))
